...
jQuery(document).ready(function () {
    ...
    if (Core.Data.get('image', false)) {
        ...
        $body.delegate('.js-resizing-radio', 'click', jQuery.proxy(Page.onSelectType, Page));
        $body.delegate('.js-resizing-input-width', 'keyup', jQuery.proxy(Page.onChangeDimensions, Page));
        $body.delegate('.js-resizing-input-height', 'keyup', jQuery.proxy(Page.onChangeDimensions, Page));
        $body.delegate('.js-resizing-button', 'click', jQuery.proxy(Page.onSubmitResize, Page));
    } else {
        bindUploadWidget();
    }
});
...
App.MainLandingPage.MainLandingPage.prototype.onChangeDimensions = function() {
    var
        $width = jQuery('.js-resizing-input-width'),
        $height = jQuery('.js-resizing-input-height'),
    ...
    jQuery('.js-resizing-button').removeClass('disabled');
};

App.MainLandingPage.MainLandingPage.prototype.onSelectType = function() {
    var $submit = jQuery('.js-resizing-button');
...
App.MainLandingPage.MainLandingPage.prototype.getResizeType = function() {
    return +jQuery('.js-resizing-radio:checked').val() === 1 ? 'resize' : 'origin';
};

App.MainLandingPage.MainLandingPage.prototype.getDimensions = function() {
    var width = jQuery('.js-resizing-input-width').val().trim(),
        height = jQuery('.js-resizing-input-height').val().trim();

    if (!width || isNaN(width)) {
        console.log('Dimensions  are not specified!');
        return false;
    }
    if (!height || isNaN(height)) {
        console.log('Dimensions  are not specified!');
...
App.MainLandingPage.MainLandingPage.prototype.onSubmitResize = function() {
    switch (this.getResizeType()) {
        case 'resize':
            ...
            this.applyEffect({effect: 'resize', options: dimensions});
            break;
        case 'origin':
            this.applyEffect({effect: 'origin'});
            break;
        default:
            break;
        jQuery('.js-resizing-button').addClass('disabled');
    }
};
...
/**
 *
 * @param {{effect: string, options: *}} params
 */
App.MainLandingPage.MainLandingPage.prototype.applyEffect = function(params) {
    params = params || {};
    // ставим лоадер
    var $imageWrapper = jQuery('.imageWrapper'),
        $loader = jQuery('<div/>').addClass('loader').css({
    ...
    if (+jQuery('.js-resizing-radio').val() === 0) {
        jQuery.ajax({
            type: 'post',
            data: {
                apply: true,
                effect: params.effect,
                additional: params.options,
                img: Core.Data.get('image')
            },
            success: function (response) {
                if (response.error) {
